...
function createElements (val) {
    ...
    const list = dataHolder.filter(element => element.completed === val);
...
function updateDel() {
    const dellButtons=document.querySelectorAll("button[type=submit]"),
...
button.addEventListener('click', ()=> {
    ...
    generateData(task);
    updateTasks();
    dataHolder.push(task);
    id++;
    input.value = '';
});
buttonAll.addEventListener('click', () => {
    contentDiv.querySelectorAll('*').forEach(n => n.remove());
    dataHolder.forEach((element)=> generateData(element));
    stage(0);
});
